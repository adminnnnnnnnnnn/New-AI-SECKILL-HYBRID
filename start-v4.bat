...
docker info >nul 2>&1
if %errorlevel% neq 0 (
    echo [错误] Docker未运�?请先启动Docker Desktop
    pause
    exit /b 1
)
...
echo.
echo [4/5] 初始化数据库...
docker exec -i seckill-mysql mysql -uroot -pyour_mysql_password seckill < seckill-parent\schema.sql
...
echo.
echo   访问地址:
echo   - Nacos控制�? http://localhost:8848/nacos
echo   - RocketMQ控制�? http://localhost:8081
echo   - MySQL: localhost:3306 (root/your_mysql_password)
